...
import "./MainnetPairPool.sol";
import "../modules/CalcuModule.sol";

contract MainnetFactory is RainbowFactory {
    ...
    function createPool(
        ...
        bytes memory bytecode = type(MainnetPairPool).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(tokenA, tokenB, pairPoolId));
        assembly {
            poolAddr := create2(0, add(bytecode, 0x20), mload(bytecode), salt)
        }
